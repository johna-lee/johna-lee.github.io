...
The script works by looping through each row in the schedule table, identifying the Match Report URL, storing it in an array, converting the array into a Pandas DataFrame, and saving the DataFrame as a CSV file. The resulting URLs for the six competitions can be found here ([match report URLs](https://github.com/johna-lee/striker-project/tree/main/match%20report%20URLs){:target="_blank"}), with the current 2024–2025 competitions included as of May 3rd, 2025.
...
* **Shot Conversion Rate** – Shot Conversion Rate is the percentage of shots that result in goals. This measures an attacker’s clinical finishing ability and is another indicator of efficiency in front of goal. Elite top scorers generally have a shot conversion rate greater than 20%.
...
![alt]({{ site.url }}{{ site.baseurl }}/assets/images/shot conversion rate.PNG)
...
#### Conclusion
* The remaining three players were added to a clustered bar chart to visualize key metrics alongside transfer value. Not only is Santiago Gimenez the top performer, but he would also cost the least.
* As such, my recommendation to Arsenal is to sign Santiago Gimenez.
